...
func (a *EmployeeApi) CreateEmployee(c *gin.Context) {
	...
	if err = utils.Verify(employee, utils.EmployeeVerify); err != nil {
		response.FailWithMessage(err.Error(), c)
		return
	}
...
// @Produce   application/json
// @Param     data  body      system.SysEmployee                                                true  "权限id, 权限名, 父角色id"
// @Success   200   {object}   "更新员工"
// @Router    /employee/updateEmployee [put]
func (a *EmployeeApi) UpdateEmployee(c *gin.Context) {
	var employee system.SysEmployee
	var err error
	...
	if employee, err = employeeService.UpdateEmployee(employee); err != nil {
		global.GVA_LOG.Error("更新失败!", zap.Error(err))
		response.FailWithMessage("更新失败"+err.Error(), c)
		return
	}
	response.OkWithDetailed(systemRes.SysEmployeeResponse{Employee: employee}, "更新成功", c)
}
...
// @Router    /employee/getEmployeeDetail [get]

func (a *EmployeeApi) GetEmployeeDetail(c *gin.Context) {
	var employee system.SysEmployee
	var err error

	// GET: /employee/getEmployeeDetail?ID=xxx
	if err = c.ShouldBindQuery(&employee); err != nil {
		response.FailWithMessage(err.Error(), c)
		return
	}

	// 详情接口只需要 ID，不做员工姓名/电话/邮箱的非空校验
	if err = utils.Verify(employee, utils.IdVerify); err != nil {
		response.FailWithMessage(err.Error(), c)
		return
	}

	detail, err := employeeService.GetEmployeeDetail(employee.ID)
	if err != nil {
...
func (a *EmployeeApi) GetEmployeeList(c *gin.Context) {
